function main()
    ...
    darwin.silverchain.generate({
        ...
        project_short_cut = "CHashManipulator",
    ...
    darwin.dtw.write_file("release/CHashManipulatorOne.c", onefile)

    local only_declare = darwin.camalgamator.generate_amalgamation("src/imports/imports.fdeclare.h", MAX_CONNTENT, MAX_RECURSION)
    only_declare = lincense .. only_declare
    darwin.dtw.write_file("release/CHashManipulator.h", only_declare)
    ...
    only_definition = '#include "CHashManipulator.h"\n' .. only_definition

    only_definition = lincense .. only_definition

    darwin.dtw.write_file("release/CHashManipulator.c", only_definition)

    os.execute("zip -r release/CHashManipulator.zip dependencies src build")
end